...
//zepto/src/data.z.js
//zepto/src/detect.js
//zepto/src/event.z.js
//zepto/src/form.js
//zepto/src/fx.js
//zepto/src/polyfill.js
//zepto/src/touch.js
//zepto/src/ie.touch.js
//zepto/src/zepto.js
...
define.pack("./core",[],function (require, exports, module) {
	...
	var getCssPrefix = function (elem, name) {
		// need
		if (preMap[name] == 1) {
			return dasherize($.fx.cssPrefix + name);
		} else if (preMap[name] == 2) {
			return name;
		} else {
			if ($(elem).css(name) !== null) {
				// 不等于null 就是说要么支持，要么已经有人设过了，直接返回
				return name;
			} else {
				// 等于null就是不支持 加前缀吧
				return dasherize($.fx.cssPrefix + name);
	...
	$.fn = {
		...
		attr: function (name, value) {
			var result;
			return (typeof name == 'string' && value === undefined) ? (this.length == 0 || this[0].nodeType !== 1 ? undefined :
		...
		val: function (value) {
			return (value === undefined) ? (this[0] &&
				(this[0].multiple ? $(this[0]).find('option').filter(function (o) {
		...
		css: function (property, value) {
			...
			if (type(property) == 'string') {
				if (!value && value !== 0)
					this.each(function () {
						this.style.removeProperty(getCssPrefix(this[0], dasherize(property)))
					})
				else
					dasherize(property)
				css = getCssPrefix(this[0], dasherize(property)) + ":" + maybeAddPx(property, value)
			}
			else {
				for (var key in property)
					if (!property[key] && property[key] !== 0)
						this.each(function () {
							this.style.removeProperty(getCssPrefix(this, dasherize(key)))
						})
					else
						css += getCssPrefix(this[0], dasherize(key)) + ':' + maybeAddPx(key, property[key]) + ';'
			}
...
define.pack("./ie",["./core"],function (require, exports, module) {
	var $ = require('./core');
	//
...
//     Zepto.js may be freely distributed under the MIT license.
	...
	if(window.scrollY === undefined) {
		Object.defineProperties(window, {
			scrollY: {
				set:function(y){
					console.warn('should using window.scrollTo insteadof setting window.scrollY');
			...
			scrollX:{
				set:function(x){
					console.warn('should using window.scrollTo insteadof setting window.scrollX');
					window.scrollTo(x, window.pageYOffset);
				},
				get:function(){
					console.warn('you are using a ie uncompatiable property window.scrollX, at your own risk');
	...
	// ie 的document.body.scrollTop也是不准的
	if(isIE) {
		Object.defineProperties(document.body, {
			scrollTop: {
				set:function(y){
					console.warn('you are using a ie uncompatiable property document.body.scrollTop, at your own risk');
					window.scrollTo(window.pageXOffset, y);
				},
				get:function(){
					console.warn('you are using a ie uncompatiable property document.body.scrollTop, at your own risk');
...
define.pack("./ajax.z",["./core"],function(require, exports, module){
	var $ = require('./core');
	...
	$.ajax = function(options){
		...
		xhr.onreadystatechange = function(){
			if (xhr.readyState == 4) {
				...
				//不支持xhr2.0
				else if(!xhr.status && (!xhr.onerror || !xhr.onabort)){
					ajaxError(null, 'xhr-1.0-error', xhr, settings)
				}
				else if(xhr.status) {
		...
		//拆分下错误逻辑
		xhr.hasOwnProperty('onerror') && (xhr.onerror = function(){
			ajaxError(null, 'onerror', xhr, settings)
		})

		xhr.hasOwnProperty('onabort') && (xhr.onabort = function(){
			!xhr.isTimeout && ajaxError(null, 'onabort', xhr, settings)
		})
		...
		if (settings.timeout > 0) 
			abortTimeout = setTimeout(function(){
				...
				xhr.isTimeout = true;
				xhr.abort()
				ajaxError(readyState, 'timeout', xhr, settings)
			}, settings.timeout)
...
define.pack("./data.z",["./core"],function(require, exports, module){
	var $ = require('./core');
...
define.pack("./detect",["./core"],function (require, exports, module) {
	var $ = require('./core');
	...
	function detect(ua) {
		...
		if (qq){
			browser.qq = true;
            var v = qq[1] || '4.5';
            //ios qq browser的版本号是xxx，没有点，5.0改掉了
            if(v.indexOf('.')<0){
                v = v[0] + '.' + v.substring(1);
            }
            browser.version = v;
		...
		if (safariLike && !qq && !uc && !wechat && !chrome && !xiaomi)
			browser.safari = true
...
define.pack("./event.z",["./core","./detect"],function (require, exports, module) {
    var $ = require('./core');
    require('./detect');
    ...
    function realEvent(type) {
        if ($.browser.ie) {
            //不要把singleTap也转义了，否则会带来巨大副作用
            if (type == 'tap') {
                type = 'click';
            }
        }
        return eventFix(hover[type] || type)
    }
                        ...
                        hasDelegate : while (true) {
                            for (var i = 0, data; data = dels[i]; i++) {
                                if ($.contains(data.element, node)) {
                                    match = $.zepto.matches(node, data.selector) ? node : null;
                                    if (match) {
                                        data.match = match;
                                        evt = $.extend(evt, {
                                            currentTarget: match,
...
define.pack("./form",["./core"],function(require, exports, module){
	var $ = require('./core');
...
define.pack("./fx",["./core"],function(require, exports, module){
    var $ = require('./core');
...
define.pack("./polyfill",[],function(require, exports, module){
	var undefined;
...
define.pack("./touch",["./core"],function (require, exports, module) {
    var $ = require('./core');
...
define.pack("./ie.touch",["./core"],function (require, exports, module) {
	var $ = require('./core');
// ie compatible
	if (!('ontouchstart' in document) && window.navigator.msPointerEnabled) {
		$.each([
			{ie: 'MSPointerDown', webkit: 'touchstart'},
			{ie: 'MSPointerMove', webkit: 'touchmove'},
			{ie: 'MSPointerUp', webkit: 'touchend'},
			{ie: 'MSPointerCancel', webkit: 'touchcancel'}
		], function (index, events) {
			$(document.body).on(events.ie, function (e) {
				// ie 只响应鼠标按下去的pointer事件，
				// ie的hover也触发MSPointerMove, 世界大坑
				if (e.type != 'MSPointerUp' && e.type != 'MSPointerCancel' && e.buttons == 0 && e.pointerType != e.MSPOINTER_TYPE_TOUCH) {
					return;
				}
				if (e.target) {
					var event = $.Event(events.webkit, {
						touches: [
							{
...
define.pack("./zepto",["./core","./ie","./ajax.z","./data.z","./detect","./event.z","./form","./fx","./polyfill","./touch","./ie.touch"],function(require, exports, module){
	var $ = require('./core');
	require('./ie');
	require('./ajax.z');
	require('./data.z');
