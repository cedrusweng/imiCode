...
var getTokens = (function() {
    ...
    /**
     * Check if a character is a decimal digit 查检字符是否为十进制数
    ...
    function parseSpaces(css) {
        ...
        // Read the string until we meet a non-space character: 读取css直到不是空格
    ...
    /**
     * Parse a string within quotes  分析引号中的字符串
     * @param {string} css Unparsed part of CSS string
    ...
    /**
     * Parse identifier 分析标识
     * @param {string} css Unparsed part of CSS string
     */
    function parseIdentifier(css) {
        var start = pos;

        // Skip all opening slashes: 跳过所有'/'杠
    ...
    /**
     * Convert a CSS string to a list of tokens 把css转换成一组符号
     * @param {string} css CSS string
     * @returns {Array} List of tokens
     * @private
     */
    function _getTokens(css, syntax) {
        var c, // current character 当前字符
            cn; // next character 下一个字符

        // Reset counters: 重置记数器
        tokens = [];
        pos = 0;
        tn = 0;
        ln = 1;
        ...
        for (pos = 0; pos < css.length; pos++) {
            ...
            // If we meet `/*`, it's a start of a multiline comment. 多行注释
